...
/* 새로운 오브젝트 만들었을때,
   ...
   공이랑 충돌 설정해주기 */
function startGame() {
  myball = new ball(40,40,5,30,100,"black"); /* width, height,ballRadius,x, y */
  goal = new goal(80,80,10,600,330);  /* width, height,ballRadius,x, y */
  startLocation = new start_location(40,40,5,30,100); /* width, height,ballRadius,x, y */
  jump_location = new jump_location(40,40,5,150,110); /* width, height,ballRadius,x, y */

  obstacle_triangle_1 = new obstacle_triangle(10,390); /* X , Y */
  obstacle_triangle_2 = new obstacle_triangle(30,390);
  obstacle_triangle_3 = new obstacle_triangle(50,390);
  obstacle_triangle_4 = new obstacle_triangle(100,390);
  obstacle_triangle_5 = new obstacle_triangle(120,390);
  obstacle_triangle_6 = new obstacle_triangle(140,390);
  obstacle_triangle_7 = new obstacle_triangle(160,390);
  obstacle_triangle_8 = new obstacle_triangle(180,390);
  obstacle_triangle_9 = new obstacle_triangle(200,390);
  obstacle_triangle_10 = new obstacle_triangle(220,390);

  obstacle_long_block_1 = new obstacle_long_block(10, 300, 70, 0); /* width, height,x, y */
  obstacle_long_block_2 = new obstacle_long_block(520, 40, 160, 260);
  obstacle_long_block_3 = new obstacle_long_block(50, 300, 350, 150);
  obstacle_long_block_4 = new obstacle_long_block(700, 10, 350, 390);
  obstacle_long_block_5 = new obstacle_long_block(100, 40, 720, 260);

  jumg_ball_1 = new jumg_ball(80,80,7,300,350); /*width, height,ballRadius,x, y */

  dash_ball_1 = new dash_ball(80,80,7,130, 300); /*width, height,ballRadius,x, y */
  dash_ball_2 = new dash_ball(80,80,7,130, 130); /*width, height,ballRadius,x, y */

  dash = new dash(80,80,5,200, 300);
  ////////////////////////////////////////////
  ///스테이지마다바뀜//////////////////////////
...
function updateGameArea() {
    ...
    if(dash_is_true == true){
    /* 만약 플레이어 공 색이 오렌지 색이면 */
    if(myball.color == "orange"){
      /* 대쉬볼을 만들어서 플레이어 공을 따라다니다가 */
      dash.x = myball.x;
      dash.y = myball.y;
      dash.update();
    ...
    /* A 키를 눌렀을 때 대쉬공이 오른쪽으로 움직인다. */
    if(myball.doAni == false && leftorRihgt == 1){
      dash.update();
      dash.newPosRight();
      myball.x= dash.x;
      myball.y= dash.y;
      ...
      if(dash_Collision_Block(dash,block_1to1) ||// 공과, myblock에 대한 충돌처리
        dash_Collision_Block(dash,block_1to2) ||
        dash_Collision_Block(dash,block_1to3) ||
        dash_Collision_Block(dash,block_1to4) ||
        dash_Collision_Block(dash,block_1to5) ||
        dash_Collision_Block(dash,block_2to1) ||
        dash_Collision_Block(dash,block_2to2) ||
        dash_Collision_Block(dash,block_2to3) ||
        dash_Collision_Block(dash,block_2to4) ||
        dash_Collision_Block(dash,block_2to5)){
          myball.doAni = true;
          leftorRihgt = 0;
        }
    ...
    /* D 키를 눌렀을 때 대쉬공이 왼쪽으로 움직인다. */
    else if(myball.doAni == false && leftorRihgt == 2){
      dash.update();
      dash.newPosLeft();
      myball.x= dash.x;
      myball.y= dash.y;
    ...
    obstacle_triangle_6.update();
    obstacle_triangle_7.update();
    obstacle_triangle_8.update();
    obstacle_triangle_9.update();
    obstacle_triangle_10.update();
...
function ball(width, height,ballRadius,x, y,color) {
  ...
  this.newPos = function() {
    ...
    if(isBall_circle_Colliding(myball,jumg_ball_1)){
      ...
      /* 대쉬 공에 부딪혔을 때 */
    if(isBall_circle_Colliding(myball,dash_ball_1)||
        isBall_circle_Colliding(myball,dash_ball_2)){
        myball.color = "orange";
      }
    ...
    if(ball_Collision_obstacle_triangle(myball,obstacle_triangle_1) ||
       ...
       ball_Collision_obstacle_triangle(myball,obstacle_triangle_6) ||
       ball_Collision_obstacle_triangle(myball,obstacle_triangle_7) ||
       ball_Collision_obstacle_triangle(myball,obstacle_triangle_8) ||
       ball_Collision_obstacle_triangle(myball,obstacle_triangle_9) ||
       ball_Collision_obstacle_triangle(myball,obstacle_triangle_10) ||
...
function jumg_ball(width, height,ballRadius,x, y) {
  ...
  /* 대쉬 볼 생성자 */
  function dash_ball(width, height,ballRadius,x, y) {
    this.width = width;
    this.height = height;
    this.ballRadius = ballRadius;
    this.x = x;
    ...
    this.update = function() {
        ...
        ctx.font = '10pt Kremlin Pro Web';
        ctx.fillStyle = "orange";
        ctx.fillText("dash" , this.x-15 , this.y-10);
        ctx.closePath();
      }
    }

    /* 대쉬  */
function dash(width, height,ballRadius,x, y) {
      this.width = width;
      this.height = height;
      this.ballRadius = ballRadius;
      this.x = x;
      ...
      this.update = function() {
          ...
          if(
          ball_Collision_Block(dash,obstacle_long_block_1) ||
          ball_Collision_Block(dash,obstacle_long_block_2) ||
          ball_Collision_Block(dash,obstacle_long_block_3) ||
          ball_Collision_Block(dash,obstacle_long_block_4) ||
          ball_Collision_Block(dash,obstacle_long_block_5) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_1) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_2) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_3) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_4) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_5) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_6) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_7) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_8) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_9) ||
          ball_Collision_obstacle_triangle(dash,obstacle_triangle_10)
        ){
          dash_is_true = false; //공을 캔버스에서 그리는 걸 멈추게한다.
          alert("fail!");
          document.location.reload();
        }
      ...
      this.newPosRight = function() {
        this.x += this.speedX;
      }

      this.newPosLeft = function() {
        this.x -= this.speedX;
      }
